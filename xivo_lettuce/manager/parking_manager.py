...
def set_parking_config(config_map):
    ...
    form.set_text_field('Extension', config_map['Extension'])

    form.set_select_field('Wait delay', config_map['Wait delay'])

    range_start = config_map['Range start']
    range_end = config_map['Range end']

    parking_range = '-'.join([range_start, range_end])
    form.set_text_field('Extension to park calls', parking_range)
