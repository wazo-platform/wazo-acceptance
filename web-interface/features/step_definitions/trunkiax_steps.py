...
@step(u'When I create an trunkiax with name "([^"]*)"')
def when_i_create_an_trunkiax_with_name_and_trunk(step, name):
...
@step(u'Then there is an trunkiax "([^"]*)"')
def then_there_is_an_trunkiax(step, name):
    _open_list_trunkiax_url()
    assert find_line(name) is not None


@step(u'Given there is an trunkiax "([^"]*)"')
def given_there_is_an_trunkiax(step, name):
    _open_list_trunkiax_url()
    try:
        find_line(name)
    except NoSuchElementException:
