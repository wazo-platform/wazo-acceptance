...
@then(u'monit monitors the service "{process_name}"')
def then_monit_monitors_the_service_process_name(context, process_name):
    until.true(context.helpers.monit.process_monitored, process_name,
               timeout=10,
               message='Monit did not monitor process {}'.format(process_name))


@then(u'monit does not monitor the service "{process_name}"')
def then_monit_does_not_monitor_the_service_process_name(context, process_name):
    until.false(context.helpers.monit.process_monitored, process_name,
                timeout=10,
