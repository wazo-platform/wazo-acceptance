...
def given_there_are_telephony_users_with_infos(context):
    ...
    for row in context.table:
        ...
        if endpoint == 'sip':
            ...
            interface_body = body.get('interface', 'autogenerated')
            if '@' in interface_body:
                pre_context, custom_context_name = interface_body.split('@')
                real_context_name = context.helpers.context.get_by(label=custom_context_name)['name']
                interface_body = f'{pre_context}@{real_context_name}'

            custom_body = {'interface': interface_body}
